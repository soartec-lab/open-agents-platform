...
type Hook<
  ...
  E extends Env,
  P extends string,
  Target extends keyof ValidationTargetsWithResponse = keyof ValidationTargetsWithResponse,
  O = {},
...
export const zValidator =
  ...
  async (c, next) => {
    if (target !== "response") {
      ...
      const baseHook = hook
        ? (
            result: Parameters<Hook<zInfer<T>, Env, string, keyof ValidationTargets, {}, T>>[0],
      ...
      return validator(c as unknown as Context<Env, string>, next) as ReturnType<
        MiddlewareHandler<E, P, V>
      >;
    }
    ...
    const { success, data, error } = (await (schema as v3.ZodType).safeParseAsync(
      value,
    )) as ZodSafeParseResult<InferredValue, Out, T>;
